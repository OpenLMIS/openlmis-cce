...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            steps {
                withCredentials([file(credentialsId: '8da5ba56-8ebb-4a6a-bdb5-43c9d0efb120', variable: 'ENV_FILE')]) {
                    ...
                    sh "docker tag openlmis/cce:latest openlmis/cce:${STAGING_VERSION}"
                    sh "docker push openlmis/cce:${STAGING_VERSION}"
                }
            }
            post {
                success {
                ...
                stage('Contract tests') {
                    steps {
                        ...
                        parameters: [
                            ...
                            text(name: 'customEnv', value: "OL_CCE_VERSION=${STAGING_VERSION}")
                        ]
                    }
                    post {
                        failure {
            ...
            steps {
                dir('erd') {
                    sh '''#!/bin/bash -xe
                        ...
                        && mkdir output \
                        && chmod 777 output \
                        && (docker run --rm --network erd_default -v $WORKSPACE/erd/output:/output schemaspy/schemaspy:snapshot -t pgsql -host db -port 5432 -db open_lmis -s cce -u postgres -p p@ssw0rd -I "(data_loaded)|(schema_version)|(jv_.*)" -norows -hq &) \
                        && sleep 30 \
                    ...
                    '''
                    archiveArtifacts artifacts: 'erd-cce.zip'
                }
            }
            post {
                failure {
        ...
        stage('Push image') {
            ...
            steps {
                sh "docker tag openlmis/cce:${STAGING_VERSION} openlmis/cce:${VERSION}"
                sh "docker push openlmis/cce:${VERSION}"
            }
            post {
                failure {
                    slackSend color: 'danger', message: "${env.JOB_NAME} - #${env.BUILD_NUMBER} ${env.STAGE_NAME} FAILED (<${env.BUILD_URL}|Open>)"
    ...
    post {
        ...
        success {
            build job: 'OpenLMIS-cce-deploy-to-test', wait: false
        }
    }
}
